...
class JobseekerMapVC: UIViewController, CLLocationManagerDelegate, GMSMapViewDelegate, UISearchBarDelegate, UITableViewDelegate, UITableViewDataSource {
    
    @IBOutlet weak var centreMarker: UIImageView! {
        didSet {
            centreMarker.layer.zPosition = 1
        }
    }
    ...
    private func setupViews() {
        ...
        searchTableView.register(UITableViewCell.self, forCellReuseIdentifier: "jobseekerSearchCell")
    }
...
//            mapView = GMSMapView.map(withFrame: view.bounds, camera: camera)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "jobseekerSearchCell", for: indexPath)
